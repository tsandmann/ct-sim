...
/**
 * <p>
 * Diese Klasse (und die von ihr abgeleiteten) abstrahieren die Datenbank, d.h. sitzen zwischen der Datenbank
 * für den Wettbewerb Oktober 2006 einerseits und ContestConductor / TournamentPlanner andererseits.
 * </p>
 * <p>
 * Diese Klasse besteht, um den ContestConductor und den TournamentPlanner um SQL zu entlasten.
 * (Ein Aufruf <code>db.setWinner(42);</code> ist übersichtlicher als ein halbes dutzend Zeilen Datenbank-Getue).
 * </p>
 * <p>
 * Alle Methoden dieser Klasse und alle Methoden der abgeleiteten Klassen werfen Instanzen von
 * java.sql.SQLException, falls die darunterliegenden Methoden, die auf die Datenbank zugreifen, eine solche
 * Exception werfen. üblicherweise deutet das darauf hin, dass die Datenbank nach einem anderen Schema aufgebaut
 * ist als der Code erwartet.
 * </p>
 *
...
class DatabaseAdapter {
	...
	/**
	 * <p>
	 * Repräsentiert die möglichen Zustände eines Spiels. Die Enum funktioniert Map-artig: Sie umfasst Paare
	 * aus einem Symbol im Code und einem String zugeordnet. Letzterer ist die Repräsentation, wie der
	...
	enum GameState {
		...
		/**
		 * <p>
		 * Diese Methode erlaubt es, Objekte dieser Klasse (dieser Enum) auf unbürokratische Art in Strings
		 * einzubauen, die SQL-Queries repräsentieren. Beispiel:
		 * </p>
	 ...
	 * @throws SQLException
	 */
	protected PreparedStatement buildPreparedStatement(String sqlWithInParams, Object... inValues) throws SQLException {
		...
		for (int i = 0; i < inValues.length; i++) {
			...
			if (inValues[i] instanceof GameState)
				rv.setString(i + 1, inValues[i].toString());
	...
	/**
	 * <p>
	 * Führt einen SQL-Befehl aus, unter Benutzung eines Caches von Befehlen. Falls der SQL-Befehl ein SELECT
	 * ist, gibt die Methode die selektierten Daten zurück.
	 * </p>
	 * <p>
	 ...
	 * </ul>
	 * Bei wiederholten Aufrufen (etwa mit wechselnden <code>ruebenId</code>-Werten) ist der Speicherbedarf im
	 * ersteren Fall linear ansteigend, da die Strings und daher die PreparedStatement-Objekte immer wieder neu
	 * erzeugt werden. Im letzteren Fall ist der Speicherbedarf konstant, da die Methode den String schon kennt
	 * und das PreparedStatement-Objekt des vorherigen Aufrufs wiederverwendet.
	 * </p>
	 * <p>
	 * <strong>Wozu?</strong> Die Methode mit ihrem Statement-Cache wurde aufgrund von Speicherlecks im ctSim
	 ...
	 * weitere SQL-Befehle sollte leicht nachzurüsten sein.
	 * </p>
	 *
	...
	throws SQLException, IllegalArgumentException {
		/*
		 * Idee: SQL-Kommando ausführen; wenn's nicht klappt, Verbindung neu herstellen und nochmal ausführen.
		 * Fix gegen abreißende DB-Verbindungen (wenn die Verbindung abreißt, bekommt man von MySQL mit dem
		 * nächsten SQL-Befehl eine com.mysql.jdbc.CommunicationsException, die von SQLException abgeleitet ist).
		 */
		try {
			return tryExecSql(sqlWithInParams, inValues);
		} catch (SQLException e) {
			lg.warn(e, "SQLException; stelle DB-Verbindung neu her und probiere nochmal");
			acquireConn();
			/**
			 * Wenn jetzt nochmal eine Exception kommt, reichen wir sie nach oben durch -- es liegt nicht an
			 * einer abgerissenen Verbindung, sondern an falscher SQL-Syntax oder so -- da hilft auch keine
			 * neue Verbindung.
			 */
			return tryExecSql(sqlWithInParams, inValues);
		}
	...
	throws SQLException, IllegalArgumentException {
		if (sqlWithInParams.toLowerCase().startsWith("select")) {
			return buildPreparedStatement(sqlWithInParams, inValues).executeQuery();
		} else if (Misc.startsWith(sqlWithInParams.toLowerCase(), "insert", "update", "delete")) {
			buildPreparedStatement(sqlWithInParams, inValues).executeUpdate();
			return null;
		} else {
			throw new IllegalArgumentException("Kann keine SQL-Befehle außer SELECT, INSERT, UPDATE und DELETE");
		}
	}

	/**
	 * Trägt einen Bot für ein Spiel ein
	 * Aufzurufen entweder vor dem Wettbewerb in der Planungsphase oder im Wettbewerb, wenn sich durch die
	 * Spielergebnisse herausstellt, welche Bots um ein Level weiterkommen.
     *
    ...
    public void placeBot(Integer botId, int levelId, int gameId) throws SQLException, TournamentPlanException {
    	...
    	lg.fine("Trage Bot ID = '" + botId + "' für Level " + levelId + " Spiel " + gameId + " ein");
