...
CREATE TABLE `ctsim_blacklist` (
  ...
  `erklärung` text collate latin1_german1_ci,
  `updatedatum` timestamp NOT NULL default CURRENT_TIMESTAMP on update CURRENT_TIMESTAMP,
  PRIMARY KEY  (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_german1_ci COMMENT='Betrifft nur Build-Skript: verbotene Schlüsselwörter' AUTO_INCREMENT=1 ;
...
CREATE TABLE `ctsim_bot` (
  ...
  `bin` mediumblob COMMENT 'Vom Build-Skript hochgeladener ausführbarer Code (ELF)',
...
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_german1_ci COMMENT='Enthält die Bots' AUTO_INCREMENT=50 ;
...
CREATE TABLE `ctsim_game` (
  `id` int(11) NOT NULL auto_increment,
  `level` int(11) NOT NULL COMMENT 'Nummer des Levels (8 für Achtelfinale, 4 für Viertelf. usw. Sonderwerte: 0 = Spiel um den 3. Platz, -1 = Vorrunde). Im Java-Code Schlüssel zusammen mit ''game''',
  `game` int(11) NOT NULL COMMENT 'Laufende Nummer des Spiels innerhalb des Levels. Im Java-Code Schlüssel zusammen mit ''level''; fängt mit 1 an (nicht 0)',
  `bot1` int(11) default NULL COMMENT 'Der eine Spieler (Fremdschlüssel ctsim_bot)',
  `bot2` int(11) default NULL COMMENT 'Der andere Spieler (Fremdschlüssel ctsim_bot)',
  `scheduled` datetime default NULL COMMENT 'Geplante Startzeit',
  `finishtime` int(11) default NULL COMMENT 'Simzeit [ms], wann das Ziel überschritten wurde',
  ...
  `state` varchar(20) collate latin1_german1_ci default 'not init' COMMENT 'mögliche Werte: "not init", "wait for bot2", "ready to run", "running", "game over"',
  `winner` int(11) default NULL COMMENT 'Gewinner (Fremdschlüssel ctsim_bot)',
  `screenshot` mediumblob COMMENT 'Zielfoto von dem Moment, wo die Ziellinie überschritten wurde. Zeigt Parcours und Bots (wie von ctSim dargestellt). Format PNG in undefinierter Größe, nur für internen Gebrauch',
  `updatedatum` timestamp NOT NULL default CURRENT_TIMESTAMP on update CURRENT_TIMESTAMP,
  PRIMARY KEY  (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_german1_ci COMMENT='Enthält die einzelnen Spiele -- siehe auch turnierbaum.pdf' AUTO_INCREMENT=575 ;
...
CREATE TABLE `ctsim_level` (
  ...
  `gametime` int(11) NOT NULL default '600000' COMMENT 'Höchstlänge [Simzeit, ms] eines Spiels auf diesem Level. Bei überschreitung wird das Spiel abgebrochen',
  `gametime_real` int(11) NOT NULL default '600' COMMENT 'Länge [Realzeit, s] eines Spiels auf diesem Level. Nur wichtig für Planung -- Spiele werden so angelegt, dass ihre Startzeitpunkte um diese Zeitspanne auseinanderliegen',
  `updatedatum` timestamp NOT NULL default CURRENT_TIMESTAMP on update CURRENT_TIMESTAMP,
  ...
  PRIMARY KEY  (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_german1_ci COMMENT='Karten und so für Achtel-, Viertel-, Halbfinale usw.';
...
CREATE TABLE `ctsim_log` (
  `id` int(11) NOT NULL auto_increment,
  `game` int(11) NOT NULL COMMENT 'Fremdschlüssel ctsim_game / Vorsicht, bezieht sich auf ctsim_game.id, NICHT auf ctsim_game.game',
  ...
  PRIMARY KEY  (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_german1_ci COMMENT='Zeigt, welcher Bot wo ist (während Spiel Insert alle 10 ms)' AUTO_INCREMENT=170607 ;
