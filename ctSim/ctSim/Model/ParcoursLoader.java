...
public class ParcoursLoader {
	...
	String[] test = { "XXXXXXXXXXXXX", "X   X       X", "X X   XXXX  X",
			"X XXXXX     X", "X     X  XXXX", "XXXX XX     X", "X     X X   X",
			"XX XXXXXXX  X" };

	private char[][] parcours = null;

	/**
	 * Erzeugt einen Wandquader Alle Postionen sind keine Weltkoordinaten,
	...
	private TransformGroup createWall(int x, int y) {
		...
		translate.set(new Vector3f((float) (x * GRID - GRID / 2), (float) (y
				* GRID - GRID / 2), 0f));
	...
	public void insertSceneGraph(TransformGroup root) {
		...
		for (int i = 0; i < test.length; i++) {
			...
			for (int j = 0; j < test[i].length(); j++)
				switch (line[j]) {
				case 'X':
					root.addChild(createWall(j - xDim / 2, yDim / 2 - i));
					break;
				}
		}
