...
//		slowMotion = false;
	...
	public void add(AliveObstacle obst){
		...
		obst.setWorld(this);
		BranchGroup bg = obst.getBranchGroup();
...
//		bg.cloneTree();
	...
	public void addBot(Bot bot) {
		...
		// TODO:
		Point3d pos = new Point3d(parcours.getStartPosition(aliveObstacles.size()+1));
		if (pos != null) {
			pos.z = bot.getPosition().z;	// Achtung die Bots stehen etwas ueber der Spielflaeche
			bot.setPosition(pos);
		}

		Vector3d head = new Vector3d(parcours.getStartHeading(aliveObstacles.size()+1));
		if (head != null) {
			bot.setHeading(head);
		}
...
//	}
	...
	/* WORLD-FUNCTIONS (for the Sensors etc.)
	 * 
	 */
	/**
	 ...
	 *            Die Blickrichtung
	 * @param openingAngle
	 *            Der Oeffnungswinkel des Blickstrahls (in Radius)
	 * @param botBody
